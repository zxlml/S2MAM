...
以下结果均由本仓库代码（seed 0）按论文协议复现：**部分标注** + **海量冗余维度** + **极端噪声维度 `N(100,100)`**，对应论文四个仿真数据集（`data/data.md`）。在全部四个实验中，S2MAM 都**精确恢复信息特征集**（recall = precision = **1.00**）并达到 **oracle 级测试性能**：

| 数据集（任务） | 数据构成 | 所选特征 | 测试性能（S2MAM vs. 基线） |
| --- | --- | --- | --- |
| 可加回归 | 8 真实 + 92 冗余 + 10 噪声，50/200 标注 | `{0,…,7}` | MSE **0.596** = oracle（全特征：50.33） |
| Friedman 回归 | 5 真实 + 95 冗余 + 10 噪声，50/200 标注 | `{0,…,4}` | MSE **3.645** = oracle（全特征：26.91） |
| 可加分类 | 2 真实 + 98 冗余 + 10 噪声，每类 10 标注 | `{0, 1}` | ACC **88.50%** = oracle（全特征：49.00%） |
| 双月牙分类 | 2 真实 + 8 冗余 + 10 噪声，每类 10 标注 | `{0, 1}` | ACC **96.00%** = oracle（全特征：50.00%） |
