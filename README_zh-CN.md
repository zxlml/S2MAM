...
严格按论文 Table 2（arXiv 2604.19072v3）协议：可加分类 `f*(x)=(x¹−0.5)²+(x²−0.5)²−0.08`，200 样本，5% 标注，损坏维度 `p_u ~ N(0,1)`、`p_n ~ N(100,100)`。我们的复现（seed 0–2）在三组含损坏设置下均与论文报告的 Test 准确率**相差不超过一个标准差**，且每次运行都精确恢复信息特征对 `{0,1}`：

| 损坏设置 | 论文（100 次重复） | 复现（3 seeds） | 差异 |
| --- | --- | --- | --- |
...
说明：干净设置（`p_u=p_n=0`，论文 90.309±3.409，复现 82.833±4.589）差距超出一个标准差，故**未列入**上表；回归与双月牙的论文定量表位于未公开的补充材料中，无法核验，同样未列入。机器可读汇总及排除理由见 [`results/summary.csv`](results/summary.csv) 与 [`results/summary_readme.txt`](results/summary_readme.txt)。

### 仓库协议结果（vs. oracle 基线）

在仓库更重的损坏设置下（92–98 冗余维、10 噪声维、回归 25% 标注 / 分类每类 10 个标注，seed 0），S2MAM 在四个仿真实验中均精确恢复信息特征集（recall = precision = 1.00）并达到 oracle 级测试性能：可加回归 MSE **0.596**（= oracle；全特征 50.33）、Friedman MSE **3.645**（= oracle；全特征 26.91）、可加分类 ACC **88.50%**（= oracle）、双月牙 ACC **96.00%**（= oracle，LapSVM 下层 + REINFORCE 引擎，`--max_outer_iter 40`）。这些数字基于我们自己的 oracle / 全特征基线，与论文表格不直接可比。
